...
export function indexOnlyScanDiagnosis(node: PlanVisualNode): IndexOnlyScanDiagnosis | null {
  ...
  if (node.heapFetches != null && node.heapFetches >= 100 && node.heapFetches >= Math.max(1, totalRows) * 0.1) return {
    ...
    evidence: `${node.heapFetches.toLocaleString()} Heap Fetches were captured for ${totalRows.toLocaleString()} returned row visits across ${node.loops.toLocaleString()} loop(s).`,
  ...
  if (removed >= 1_000 && removed >= Math.max(1, totalRows)) return {
    ...
    evidence: `${removed.toLocaleString()} rows were removed by a residual filter versus ${totalRows.toLocaleString()} returned row visits.`,
  ...
  if (node.loops >= 100 && (node.timeShare >= 10 || node.totalTime >= 100)) return {
    ...
    evidence: `${node.loops.toLocaleString()} loops accumulated ${node.totalTime.toLocaleString(undefined, { maximumFractionDigits: 2 })} ms (${node.timeShare.toFixed(1)}% of execution).`,
  ...
  if (node.heapFetches === 0) return {
    ...
    evidence: `Heap Fetches = 0; ${node.loops.toLocaleString()} loop(s) accumulated ${node.totalTime.toLocaleString(undefined, { maximumFractionDigits: 2 })} ms.`,
