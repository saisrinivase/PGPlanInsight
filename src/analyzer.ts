...
export function analyzePlan(source: string): Analysis {
  ...
  const indexOnlyReview = planMap.map((node) => ({ node, diagnosis: indexOnlyScanDiagnosis(node) })).find((item) => item.diagnosis?.review);
  ...
  const primarySignal = !runtime ? "Capture quality" : elevatedWorkMem ? "Capture guardrail" : planningDominates ? "Planning overhead" : spill ? "Memory / spill" : writeWal ? "WAL / write pressure" : largeOffset ? "Pagination work" : parallelSuppressed ? "Parallelism" : badEstimate ? "Estimate drift" : highLoops ? "CPU loop amplification" : seqScan ? "Access path" : coercions.length ? "Type coercion" : "No dominant risk detected";
