...
export function CandidateIndexExperiments({ result, context }: { result: Analysis; context: DatabaseContext | null }) {
  ...
  return <section className="candidate-experiment-workbench" aria-label="Controlled candidate index experiments">
    ...
    <div className="candidate-experiment-list">{experiments.map(({ node, experiment }, index) => <details key={`${node.path}-${experiment.candidateShape}`} open={index === 0}><summary><span>{String(index + 1).padStart(2, "0")}</span><span><small>{experiment.status.replaceAll("-", " ")} · {experiment.confidence} confidence</small><strong>{experiment.candidateShape}</strong><em>Operation {node.rank} · {node.nodeType} · {node.sharedReads.toLocaleString()} reads · {node.timeShare.toFixed(1)}% inclusive time</em></span><b>Review experiment</b></summary><div className="candidate-experiment-body">
      ...
      <section className="hypopg-experiment" aria-label="Index recommendation decision"><h4>Should I create another index?</h4><p>{experiment.decision}</p><p>Runtime improvement: unknown. Estimated planner cost is not elapsed time and cannot establish a percentage speedup.</p>{experiment.testSql ? <><h4>Sample SQL for a DBA-reviewed, non-production test</h4><code>{experiment.testSql}</code><p>This creates a physical index and can block writes. Review operational impact before running it. This application does not execute SQL.</p></> : <p>No physical CREATE INDEX statement is offered for this evidence state. Confirm a schema-qualified relation before testing.</p>}</section>
      {experiment.testSql && <section className="hypopg-experiment"><header><div><small>Optional planning-only validation</small><h4>HypoPG experiment</h4></div><span>Extension availability unknown</span></header><code>{experiment.hypopg.create}</code><p>{experiment.hypopg.inspect}</p><code>{experiment.hypopg.rollback}</code></section>}
