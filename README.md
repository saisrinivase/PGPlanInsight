...
- IndexedDB execution-plan history retained until manual clear
...
The gate runs unit tests, a production TypeScript/Vite build, Chromium/Firefox/WebKit desktop and mobile workflows, static secret and unsafe-sink checks, the offline production dependency advisory audit, license policy, SBOM generation, and SHA-256 artifact hashing. Local evidence is written to `dist/release/`; CI verifies it but does not upload it.
